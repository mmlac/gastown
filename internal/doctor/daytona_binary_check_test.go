...
func TestDaytonaBinaryCheck_DaytonaNotInPath(t *testing.T) {
	...
	if result.Status != StatusError {
		t.Errorf("expected StatusError when daytona is not in PATH, got %v: %s", result.Status, result.Message)
	}
	...
	if !strings.Contains(result.FixHint, "daytona.io") {
		t.Errorf("fix hint should reference daytona.io, got %q", result.FixHint)
	}
}
...
func TestDaytonaBinaryCheck_DaytonaTooOld(t *testing.T) {
	...
	result := check.Run(ctx)
	if result.Status != StatusError {
		t.Errorf("expected StatusError for too-old daytona, got %v: %s", result.Status, result.Message)
	}
	if !strings.Contains(result.Message, "too old") {
...
func TestDaytonaBinaryCheck_DaytonaVersionFails(t *testing.T) {
	...
	result := check.Run(ctx)
	if result.Status != StatusError {
		t.Errorf("expected StatusError when daytona version fails, got %v: %s", result.Status, result.Message)
	}
